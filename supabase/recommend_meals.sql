...
AS $function$
  with u as (
    select
      ...
      -- CHANGED: ordered favorites, fallback to single pref_cuisine_id so nothing breaks pre-migration
      coalesce(
  ...
  scored as (
    select
      ...
      -- CHANGED: cuisine term now ranked 30/20/10 by favorite position
      + case array_position(u.fav_ids, m.cuisine_id)
          when 1 then 30
          when 2 then 20
          when 3 then 10
          else 0
      ...
      -- mood adjustment (raised so it can flip rank; cuisine untouched so taste stays primary; NULL mood = no effect)
      + case lower(coalesce(p_mood, ''))
          ...
          -- CHANGED: comfort/adventurous key off top favorite (fav_ids[1])
          when 'comfort' then case when m.cuisine_id = u.fav_ids[1] then 12 else 0 end
          when 'adventurous' then case when u.fav_ids[1] is null or m.cuisine_id <> u.fav_ids[1] then 14 else 0 end
          else 0
        end
        as score
    from meals m
